...
const ThreeJSGallery = ({ projects, onSelectProject }) => {
  ...
  useEffect(() => {
    ...
    projects.forEach((project, i) => {
      // 画框
      const frameGeometry = new THREE.BoxGeometry(w + thickness * 2, h + thickness * 2, thickness);
      ...
      const painting = new THREE.Mesh(new THREE.PlaneGeometry(w, h), paintingMaterial);
  
      // GitHub 图标 - 使用预加载的纹理
      const githubTexture = texturesRef.current["public/textures/github-mark.png"];
      if (githubTexture) {
        const iconSize = 0.25;
  ...
  useEffect(() => {
    ...
    const handleClick = (e) => {
      ...
      if (intersects.length > 0) {
        ...
        } else if (clickedObject.parent && clickedObject.parent.userData.projectId) {
  ...
  }, [renderer, camera, paintings, raycaster, mouse, selectedPainting, onSelectProject, texturesLoaded]);
      ...
      {!texturesLoaded && (
        <div className="absolute inset-0 flex items-center justify-center bg-gray-100 bg-opacity-70 rounded-lg z-10">
          <div className="text-center">
            ...
            <p className="mt-2 text-gray-700">加载资源中... {loadingProgress}%</p>
          </div>
        </div>
      )}
    </div>
